...
async function joinVoiceChannel(channel) {
  ...
  try {
    ...
    alert("Mikrofonzugriff verweigert oder kein Mikrofon gefunden!");
    return;
  }
  ...
  const emitJoin = (pId) => {
    ...
    socket.emit("join_voice_channel", { channelId: channel.id, peerId: pId });
    $("#voiceControl")?.classList.remove("hidden");
    if ($("#voiceStatus")) $("#voiceStatus").textContent = `Verbunden: ${channel.name}`;
  };

  if (peer.id) emitJoin(peer.id);
  else peer.on("open", (pId) => emitJoin(pId));
}

function leaveVoiceChannel() {
  if (!currentVoiceChannelId) return;

  socket.emit("leave_voice_channel", { channelId: currentVoiceChannelId, peerId: peer?.id });
...
function setupVoiceSocketEvents() {
  ...
  socket.on("user_joined_voice", ({ peerId }) => {
...
function renderChannelList() {
  ...
  cat.querySelector("#addChannelBtn").onclick = async () => {
    ...
    const isVoice = confirm("Soll das ein Sprachkanal sein?\n[OK] = Sprachkanal (Voice)\n[Abbrechen] = Textkanal");
    const type = isVoice ? "voice" : "text";

    await api("/api/channels", { method: "POST", body: JSON.stringify({ serverId: state.server.id, name, type }) });
    const d = await api("/api/bootstrap");
    state.servers = d.servers;
...
function upsertMessage(m) {
  ...
  if (m.id != null && wrap.querySelector(`.msg[data-id="${m.id}"]`)) return;
